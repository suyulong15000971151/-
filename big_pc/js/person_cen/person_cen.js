...
if(type != null && type == 7) {
	$(".conta .con-box .sidebar .list .active").removeClass("active");
	$(".conta .con-box .sidebar .list .apiTransaction").addClass("active");

	$(".conta .con-box .sidebar .list .apiTransaction").parent().find("i").addClass("tv");
	$(".conta .con-box .sidebar .list .apiTransaction").parents(".list").siblings().find("i").removeClass("tv");
	...
	$.ajax({
		type: 'GET',
		url: PCData.APPUSER_URL + "appuser/queryapiinfo?bauId=" + pcUser.id,
		contentType: "application/json",
		dataType: 'json',
		success: function(data) {
			if(data.code == "200") {
				var userApi = data.data;
				if(userApi != null) {
					$(".bindSetApi").hide();
					$(".openApiBtn").hide();
					$(".openApi").show();
					...
					$(".apiSet").hide();
					var accessKey = '<p >API访问密钥(Access Key)： ' + userApi[0].uuid + '</p><p id="secretKey">API私有密钥(Secret Key)：******************************************* </p>'
					$(".generateApiDiv").html(accessKey);
					if(userApi[0].ip != null && userApi[0].ip != "") {
						var ipId = '<p><td>' + userApi[0].ip + '</td></p>'
						$(".modifyApiDiv").html(ipId);
					}
					else {
						var ipId = '<p>不绑定ip，不限制<td></td></p>'
						$(".modifyApiDiv").html(ipId);
					}
				}
			}
...
$(function() {
	...
	$("#loginModifyDiv").click(function() {
		...
		$(this).parents(".item").siblings().find(".set").text("修改");

		if(pcUser.googleStatus != 1) {
			$("#googleModigyDiv").text("设置");
		} else {
			$("#googleModigyDiv").text("查看");
		}

		var display = $('#modifyFundPwdDiv').css('display');
		if(display == 'none') {
			if(pcUser.fundsStatus != 1) {
				$("#fundModifyDiv").text("设置");
			} else {
				$("#fundModifyDiv").text("修改");
			}
		} else {
			$("#fundModifyDiv").text("取消");
		}

		if(pcUser.mailbox == null || pcUser.mailbox == "" || pcUser.mailbox.length < 1) {
			$("#emailModifyDiv").text("设置");
		} else {
			$("#emailModifyDiv").text("修改");
		}
		if(block == "block") {
			$(this).text("修改");
		} else {
	...
	$(".bindApi .open").click(function() {
		phone = pcUser.phone;
		var regs = /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/;
		var apiId = $("#apiId").val();
		if(apiId != null && apiId != "") {
			if(!regs.test(apiId)) {
				Message("API交易IP格式不正确", "error");
				return false;
			}
		}
		//验证码
		var vaildatecode = $('#apiMethodText').val();
		...
		$.ajax({
			...
			success: function(data) {
				if(data.code != "200") {
					...
					$.ajax({
						type: 'GET',
						url: PCData.APPUSER_URL + 'appuser/getrsa?bauId=' + pcUser.id + "&ip=" + apiId,
						async: false,
						success: function(data) {
							if(data != null) {
								if(data.code == "200") {
									...
									$(".openApiBtn").hide();
									$(".closeApiBtn").show();
									var accessKey = '<p >API访问密钥(Access Key)： ' + data.data.accessKey + '</p><p id="secretKey">API私有密钥(Secret Key)：' + data.data.secretKey + '  [仅显示一次，若遗忘请重新生成]</p>';
									$(".generateApiDiv").html(accessKey);
									if(apiId != null && apiId != "") {
										var ipId = '<p><td>' + apiId + '</td></p>'
										$(".modifyApiDiv").html(ipId);
									} else {
										var ipId = '<p>不绑定ip，不限制<td></td></p>'
										$(".modifyApiDiv").html(ipId);
					...
					success: function(data) {
						if(data.code == "200") {
							...
							if(userApi != null) {
								...
								$(".bindCloseApi").hide();
								$(".titleText").html("API私有交易设置");
								var accessKey = '<p >API访问密钥(Access Key)： ' + userApi[0].uuid + '</p><p id="secretKey">API私有密钥(Secret Key)：******************************************* [仅显示一次，若遗忘请重新生成]</p>'
								$(".generateApiDiv").html(accessKey);
								if(userApi[0].ip != null && userApi[0].ip != "") {
									var ipId = '<p><td>' + userApi[0].ip + '</td></p >'
									$(".modifyApiDiv").html(ipId);
	...
	$("#bankModifyDiv").click(function() {
		...
		if(pcUser.payment != null && pcUser.payment.paymentMethod1 != null && pcUser.payment.paymentMethod1 === 0) {
			$("#aliModifyDiv").text("修改");
		} else {
			$("#aliModifyDiv").text("设置");
		}
	...
	$("#wxModifyDiv").click(function() {
		...
		if(pcUser.payment != null && pcUser.payment.paymentMethod3 != null && pcUser.payment.paymentMethod3 === 0) {
			$("#bankModifyDiv").text("修改");
		} else {
			$("#bankModifyDiv").text("设置");
		}
...
if(pcUser.phone != null && pcUser.phone != "") {
	...
	$("#authenPhone").append('<span class="set" onclick="goSecurity(1);">去设置</span>');
}

if(pcUser.mailbox != null && pcUser.mailbox != "") {
	$("#authenEmail").append('<span>' + pcUser.mailbox + '</span>');
} else {
	$("#authenEmail").append('<span class="set" onclick="goSecurity(2);">去设置</span>');
}

if(pcUser.name != null && pcUser.name != "") {
	$("#authenName").append('<span>' + pcUser.name + '</span>');
} else {
	$("#authenName").append('<span class="set" onclick="goAuth();">去认证</span>');
}
...
function sendCode(codeType) {
		...
		if(phone == "") {
			...
			} else if(codeType != null && codeType == 10) {
				var reg = /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/;
				api = $("#apiId").val();
				if(api != null && api != "") {
					if(!reg.test(api)) {
						$("#apiIdTiShi").text("API交易IP格式不正确");
						return false;
					} else {
						$("#apiIdTiShi").text("");
					}
			...
			} else if(codeType != null && codeType == 11) {
				var regs = /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/;
				api = $("#updateIP").val();
				if(api != null && api != "") {
					if(!regs.test(api)) {
						$("#updateIpTiShi").text("API交易IP格式不正确");
						return false;
					} else {
						$("#updateIpTiShi").text("");
					}
...
function codeTimeInit(codeType) {
	if (codeType != sendCodeType) {
	if(sendCodeType != null && sendCodeType == 2) {
		$('#modifyPasswd').html('发送验证码');
		$('#passwordText3').html("");
	} else if(sendCodeType != null && sendCodeType == 3) {
		$('#modifyPhone').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 4) {
		$('#modifyGoogle').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 5) {
		$('#modifyFundpw').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 6) {
		$('#modifyEmail').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 7) {
		$('#bankMethodText').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 8) {
		$('#wxMethodText').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 9) {
		$('#alMethodText').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 10) {
		$('#apiMethod').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 11) {
		$('#updateMethod').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 12) {
		$('#againMethodText').html('发送验证码');
	} else if(sendCodeType != null && sendCodeType == 13) {
		$('#closeMethodText').html('发送验证码');
	}
...
codeId = null;
			...
			if(index == 0) {
				if(codeType != null && codeType == 2) {
					...
					$('#updateMethod').html('发送验证码');
				} else if(codeType != null && codeType == 12) {
					$('#againMethodText').html('发送验证码');
				}else if(codeType != null && codeType == 13) {
					$('#closeMethodText').html('发送验证码');
				}
			} else {
				if(codeType != null && codeType == 2) {
					...
					$('#apiMethod').html('<span style="color:#0A5BEF">' + index + 'S</span>');
				} else if(codeType != null && codeType == 11) {
					$('#updateMethod').html('<span style="color:#0A5BEF">' + index + 'S</span>');
				} else if(codeType != null && codeType == 12) {
					$('#againMethodText').html('<span style="color:#0A5BEF">' + index + 'S</span>');
				}else if(codeType != null && codeType == 13) {
					$('#closeMethodText').html('<span style="color:#0A5BEF">' + index + 'S</span>');
				}
			}
			index--;
		}
...
function updateIp() {
	...
	var apiId = $("#updateIP").val();
	//验证码
	var vaildatecode = $('#updateMethodText').val();
	var isNum = PCUtils.checkStr(vaildatecode, 3);
	if(!isNum) {
		Message("验证码格式不正确", "error");
		return false;
	...
	$.ajax({
		...
		success: function(data) {
			if(data.code != "200") {
				...
				$.ajax({
					type: 'GET',
					url: PCData.APPUSER_URL + 'appuser/bindip?bauId=' + pcUser.id + "&ip=" + apiId,
					async: false,
					success: function(data) {
						if(data != null) {
							if(data.code == "200") {
								...
								obj.innerHTML = "API私有密钥(Secret Key)：******************************************* [仅显示一次，若遗忘请重新生成]";
								$("#updateMethodText").val("");
								$("#updateIP").val("");
							} else {
								Message(data.message, "error");
	...
	$.ajax({
		...
		success: function(data) {
			if(data.code != "200") {
				...
				$.ajax({
					type: 'GET',
					url: PCData.APPUSER_URL + 'appuser/getrsa?bauId=' + pcUser.id + "&ip=",
					async: false,
					success: function(data) {
						if(data != null) {
							if(data.code == "200") {
	...
	$.ajax({
		...
		success: function(data) {
			if(data.code != "200") {
				...
				$.ajax({
					type: 'GET',
					url: PCData.APPUSER_URL + 'appuser/delrsa?bauId=' + pcUser.id,
					async: false,
					success: function(data) {
						if(data != null) {
							if(data.code == "200") {
								Message("API关闭成功", "win");
								setTimeout('window.location.href="../../view/person_cen/person_cen.html?type=7";', 300);
							} else {
								Message(data.message, "error");
							}
						}
