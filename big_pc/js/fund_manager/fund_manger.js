				...
				if(!!res.data) {
					...
					for(i = 0; i < list.length; i++) {
						...
						if(list[i].rechargeStatus != 1 || list[i].userrechargeStatus != 1) {
							strR = "<td><span style=\"color:#ccc\" >充币</span>";
						} else {
							strR = "<td><span onclick=\"rechargeClick(" + list[i].tokenId + ");\">充币</span>";
						}

						var strW = "";
						if(list[i].withdrawalsStatus != 1 || list[i].userwithdrawStatus != 1) {
							strW = "<span style=\"color:#ccc\">提币</span>";
						} else {
							strW = "<span onclick=\"withdrawClick(" + list[i].tokenId + ",'" + list[i].tokenName + "');\">提币</span>";
						}
							...
							success: function(res) {
								if(res != null && res.code == "200" && res.data != null) {
									if(res.data.transactionStatus != 1) {
										strB = "<span style=\"color:#ccc\">交易</span></td></tr>";
									} else {
										strB = "<span onclick=\"tradeClick(" + list[i].tokenId + ",'" + list[i].tokenName + "');\">交易</span></td></tr>";
									}
								} else {
									strB = "<span style=\"color:#ccc\">交易</span></td></tr>";
								}
...
function confirmWithdraw() {
    ...
    if(baaId==""){
	    ...
	    if(addr.replace(/(^s*)|(s*$)/g, "").length ==0){
	    	Message("提币地址不能为空", "error");
	    }else if(reg.test(addr)){
	    	Message("提币地址不能包含汉字", "error");
	    }else{
	    	var bindData={
				"bauId": userId,
				"address":addr,
   ...
   if(baaId!=null&&baaId!=""){
   	    ...
   	    if(vaildatecode==null||vaildatecode.trim()==""){
   	    	...
   	    	Message("验证码必须为数字", "error");
   	    }else if(num<txNum){
   	    	Message("提币数量应大于最小提币数量", "error");
   	    }else{ 
   	    		var forgetpwdAuthD = {
					"phone": pcUser.phone,
					...
					success: function(forgetpwdAuthD) {
						if(forgetpwdAuthD.code != "200") {
						   ...
						   Message("提币地址不能为空", "error");
			   	    	    ...
			   	    	    Message("提币数量不能为空", "error");
			   	        }else if(!PCUtils.checkStr(num,9)){
			   				Message("提现数量必须为数字", "error");
			   		   }else{
			   		   		var data = {
								"bau_id": userId,
		...
		success: function(res) {
			...
			if(res.code == "200" && res.data != null && res.data.objectList != null) {
				...
				for(i = 0; i < list.length; i++) {
					...
					if(tabIndex == 0) {
						//买入
						_operateHtml = "<td class='undo' onclick='undo(\"" + tradingArea + "\",\"" + token + "\"," + type + "," + userId + "," + list[i].id + "," + page + "," + rows + ")'>撤单</td>";
					} else {
						_operateHtml = "<td>--</td>";
					}
					_css = "";
		...
		success: function(chargRes) {
			if(chargRes.code == "200" && chargRes.data != null) {
				...
				if(getWithdrawType!=null){
					$("#numWithdraw").attr('placeholder', "最小提币数量为："+getWithdrawType+tokenName);
					txNum=getWithdrawType;
				}else{
					$("#numWithdraw").attr('placeholder', "最小提币数量为：0"+tokenName);
				}
			} else {
				$("#numWithdraw").attr('placeholder', "最小提币数量为：0"+tokenName);
			}
