...
if(tradeListWebsocket == null) {
	console.log("Browsers do not support websocke");
}
...
$("#RealTimeTransactionTradingAreak").append("price(" + tradingAreak + ")")
$("#RealTimeTransactionToken").empty();
$("#RealTimeTransactionToken").append("number(" + tokenk + ")")
$(document).ready(function() {

	$("#showPrice").html("price( " + tradingAreak + " )");
	$("#showNum").html("number( " + tokenk + " )");
	$(".showPrices").html("price( " + tradingAreak + " )");
	$(".showNums").html("number( " + tokenk + " )");
	...
	klineWebsocket.onmessage = function(event) {
		...
		$(".shares .shares_title .gao").text("high"+" " + res.data.high.toFixed(6))
		$(".shares .shares_title .di").text("low"+" " + res.data.low.toFixed(6))
		$(".shares .shares_title .amount").text("24H amount"+" " + res.data.vol.toFixed(6)+" "+token)
	};
	if(!!refKL) {
		clearInterval(refKL);
	}
...
function strJoinFn(r, num) {
	...
	if(inList == null || inList.length < 1) {
		for(var i = 0; i < num; i++) {
			var create_tr = $('<tr></tr>');
			var _html = '<td>buy' + (i + 1) + '</td><td></td><td></td>';
		...
		if(inList.length < num) {
			for(var i = 0; i < inList.length; i++) {
				...
				var _html = '<td>buy' + (i + 1) + '</td><td>' + inList[i].unitPrice.toFixed(6) + '</td><td>' + inList[i].unfinishedcount.toFixed(6) + '</td>';
				create_tr.html(_html);
				$('#inResultBox').append(create_tr);
	...
	if(outList == null || outList.length < 1) {
		for(var i = num - 1; i >= 0; i--) {
			var create_tr = $('<tr></tr>');
			var _html = '<td>sell' + (i + 1) + '</td><td></td><td></td>';
			create_tr.html(_html);
		...
		if(outList.length < num) {
			...
			for(var i = outList.length - 1; i >= 0; i--) {
				...
				var _html = '<td>sell' + (i + 1) + '</td><td >' + outList[i].unitPrice.toFixed(6) + '</td><td>' + outList[i].unfinishedcount.toFixed(6) + '</td>';
				create_tr.html(_html);
				$('#outResultBox').append(create_tr);
			};
		} else {
...
function strJoinRT(r) {
	...
	for(var i = 0; i < realTimeList.length; i++) {
		var create_tr = $('<tr></tr>');
		var _html = '<td>' + PCUtils.getTimeFull(realTimeList[i].updateTime, "-") + '</td><td>' + (realTimeList[i].type != null && realTimeList[i].type == 0 ? "<div class='win'>Purchase</div>" : realTimeList[i].type != null && realTimeList[i].type == 1 ? "<div class='fault'>Sell out</div>" : "") + '</td>' +
